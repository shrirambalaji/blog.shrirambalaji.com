...
I also had some interesting things I wanted to try and making a blog felt like the perfect starting point:
...
Since my portfolio was also on tailwind, it was pretty straightforward to port styles. I just had to update a couple of `css-variables` and replace the fonts to use `Inter` (probably the best font ever, thanks [@rsms](https://twitter.com/rsms)). I still had to make some more stylistic changes to make the starter attuned to my liking (for eg. appearance of tags, switching from a monospace font to sans-serif font for readability etc.,)

But I ran into some troubles with code highlighting. The default Astro + Shiki configuration didn't allow me to switch themes.
...
[Obsidian-git](https://github.com/denolehov/obsidian-git) is a plugin that has a bunch of nice git features which supercharge an Obsidian vault with features like: auto backup, sync, pull, push, fetch, clone and treat any git repository as a vault.
...
**Astro** and the ecosystem around it has enabled me to make this change in a couple of days. Looking forward to write more here. Let's see ✌️
