...
As a workaround, I tried the [`File Hider`](https://github.com/Oliver-Akins/file-hider) plugin to hide the rest of the `src` folder and now the setup was ready. I wanted to write [`MDX`](https://mdxjs.com/) to use Astro components in my blogposts, so I also had to install [mdx-as-md](https://github.com/git-no/obsidian-markdown-file-suffix) which recognizes MDX files inside of obsidian.
