...
On linux machines, the de-facto toolchain is `gcc`(GNU Compiler Collection) which is missing. The `build-essential` meta package has `gcc` and a couple other packages.

```shell
$ sudo apt install build-essential
```

However, we can choose a different linker like `lld` which
