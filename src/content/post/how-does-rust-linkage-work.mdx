...
Rust prefers the opposite called [static linking](https://en.wikipedia.org/wiki/Static_library) wherein it will link in all Rust and native dependencies, producing a single distributable binary. This is called a `bin` crate in rust, and is the default type of crate we get when we initialize a new project with `cargo`
