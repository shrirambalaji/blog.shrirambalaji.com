---
title: Contributing to the Rust Project
...
`panic-unwind` is enabled by default, btw. The internal `toml` parser, parses all the fields in a predefined `struct` from the config file, so it was as easy as adding a new field in there.
...
I addressed all the feedback comments, and the PR was ready for merge.
